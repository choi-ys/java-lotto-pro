...
# 영상 피드백 적용
### 영상 : 로또 TDD 강의 및 일급 콜렉션 적용 리팩토링
```
  - 객체의 행위를 표현하는 메서드 구성을 통해 get 메서드를 이용한 외부 접근이 아닌, 객체 내부로 메시지를 전달
  - 일급컬렉션, 문자열과 원시 타입 포장 객체를 이용하여 역할과 책임을 분리
    - 일급 컬렉션과 원시타입 포장객체를 이용하여 역할과 책임을 분리한 경우, 해당 객체에 대한 유효성 검증과 행위 표현에 집중할 수 있음
...
- Lotto 객체 리펙토링
  ...
  - Lotto 객체 필드의 자료구조 변경
    ...
    - 변경 사유 : List로 이루어진 6개의 로또 번호의 중복 여부 확인을 위해 Set으로 변경 하여 size를 체크 후, 다시 List로 구성된 필드에 값을 할당하는 비효율적인 작업 개선
...
### 영상 : private method에 대한 테스트 및 리팩토링
```
...
### 영상 : 객체 간 의존관계 연결, 인자 수를 최소화
```
메서드의 인자수를 최소화한다 (클린 코드 내용 중)
- 메서드에서 이상적인 인자 개수는 0개이다. 다음은 1개이고, 다음은 2개이다.
3개는 가능한 피하는 편이 좋다. 4개 이상은 특별한 이유가 있어도 사용하면 안 된다.

클래스를 분리하기 위한 정량적인 기준
- 일급 컬렉션, 원시 타입 포장 객체, 메서드의 인자를 줄이기 위한 객체 생성
- 일급 컬렉션을 구현할 때 접근방법으로 상속과 조합 방법으로 구현할 수 있다.

is-a(상속), has-a(조합)을 통한 객체 간 의존관계 연결
- 객체의 중복(Lotto와 WinningLotto)을 제거할 때, 상속과 조합 방법을 고려할 수 있다.
- 코드의 재 사용성 측면에서는 상속이 유리하지만, 유연성 측면에서는 조합이 더 유리하다.
- 변화에 빠르게 대응하는 것이 점점 더 중요해지는 현재 관점에서는 재 사용성보다 유연성이 더 중요할 수 있다.
- 상속은 부모가 정의한 interface의 변경이 자식에게 주는 영향도를 고려했을 때 유지보수가 힘들 수 있는 점을 인지해야 한다.
- 상속을 활용하는 것이 명확하다는 판단이 있기 전까지는 조합을 이용하여 유연성을 확보하는 것을 권장한다.
```

### 영상 : 정적 팩토리 메소드, 인스턴스 캐싱
```
고정된 범위와 용도를 가지는 객체의 인스턴스 생성 시, 메모리 사용 개선
```
리팩토링 내용
- LottoNumber 객체의 인스턴스 생성 시 메모리 효율 개선
  - 접근 : 로또 번호는 1~45의 고정된 값을 가진다. 
  - 상황 :
    - 로또 1게임 진행에 필요한 로또 번호는 6개이다. 
    - 로또 1장은 5개의 로또 게임으로 구성되어 있다.
    - 로또 1장을 표현하기 위해 필요한 LottoNumber 객체는 30개이다. 
    - 동시에 만 명의 사용자가 로또 1장씩 구매하는 경우, 30만 개의 LottoNumber 객체가 생성된다.
  - 고민 : 변하지 않는 범위와 용도를 가지는 객체를 필요한 만큼 생성하고, GC에 의해 해제되는 것이 효율적인 메모리 사용인지 고민 
  - 접근 : 변하지 않는 범위와 값을 가지는 LottoNumber 객체를 Heap 영역에 미리 생성하여 매 요청 시, 이미 생성된 인스턴스 재활용을 통해 메모리 사용 개선
  - 효과 : 동시에 만 명의 사용자가 로또 1장씩 구매하는 경우, 30만 개의 LottoNumber 객체가 생성되는 것이 아닌 45개의 LottoNumber 객체가 생성
  - 검증 : 객체 동일성 비교 TC를 통해 효과를 검증
